                    ...
                    public boolean verify(String hostname, SSLSession session) {
                        ...
                        for (Certificate peerCert: peerCerts) {
                            if (peerCert instanceof X509Certificate) {
                                X509Certificate x509Cert = (X509Certificate) peerCert;
                                Principal x500s = x509Cert.getSubjectX500Principal();
                                Map<String, String> subjectMap = KeyStoreUtil.getX500Map(x500s);

                                if (Debug.infoOn()) {
                                    Debug.logInfo(x509Cert.getSerialNumber().toString(16) + " :: " + subjectMap.get("CN"), module);
                                }
